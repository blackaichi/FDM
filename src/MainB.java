...
class MainB {
    private static final double indexFibra = 1.47;
    private static int regio = 0;
    private static int nreg;
    private static double angleF = 0;
    private static boolean up = true;
    private static double posX;
    private static double posY;
    private static int alpha;
    private static int pointsxU;
    ...
    private static double[][] getData(int mode) {
        double[] xData;
        double[] yData;
        if (mode == 0) {
    ...
    private static void downThings(double[] xData, double[] yData, int i) {
        yData[i] = posY - 1;
    ...
    private static void upThings(double[] xData, double[] yData, int i) {
        yData[i] = posY + 1;
        xData[i] = posX + (Math.tan(Math.PI * angleF / 180));
        regio++;
        posY++;
    ...
    private static double calculIndexRegio(double r, double a, int alfa) {
        double delta = 0.01;
        return indexFibra*Math.sqrt(1-2*delta*Math.pow((Math.abs(r)/a),alfa));
    }

    private static double snell(double n1, double angleIncident, double n2) {
        //System.out.println("n1 = " + n1 + ", angI = " + angleIncident + ", n2 = " + n2);
        return Math.asin(n1*Math.sin(Math.PI*angleIncident/180)/n2)*180/Math.PI;
    }

    private static double angleCritic(double n1, double n2) {
